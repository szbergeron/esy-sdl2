...
static void touch_add(SDL_TouchID id, float x, float y,
                      struct wl_surface *surface) {
  struct SDL_WaylandTouchPoint *tp =
      SDL_malloc(sizeof(struct SDL_WaylandTouchPoint));
...
static void touch_del(SDL_TouchID id, float *x, float *y) {
  struct SDL_WaylandTouchPoint *tp = touch_points.head;
...
static SDL_bool ProcessHitTest(struct SDL_WaylandInput *input,
                               uint32_t serial) {
  SDL_WindowData *window_data = input->pointer_focus;
  SDL_Window *window = window_data->sdlwindow;

  if (window->hit_test) {
    const SDL_Point point = {wl_fixed_to_int(input->sx_w),
                             wl_fixed_to_int(input->sy_w)};
    const SDL_HitTestResult rc =
        window->hit_test(window, &point, window->hit_test_data);
    ...
    case SDL_HITTEST_DRAGGABLE:
      if (input->display->shell.xdg) {
        ...
        wl_shell_surface_move(window_data->shell_surface.wl, input->seat,
                              serial);
      }
      return SDL_TRUE;
    ...
    case SDL_HITTEST_RESIZE_LEFT:
      if (input->display->shell.xdg) {
        ...
        zxdg_toplevel_v6_resize(
            window_data->shell_surface.zxdg.roleobj.toplevel, input->seat,
            serial, directions_zxdg[rc - SDL_HITTEST_RESIZE_TOPLEFT]);
      } else {
        wl_shell_surface_resize(window_data->shell_surface.wl, input->seat,
                                serial,
                                directions_wl[rc - SDL_HITTEST_RESIZE_TOPLEFT]);
      }
      return SDL_TRUE;
...
static void pointer_handle_button_common(struct SDL_WaylandInput *input,
...
/*struct SDL_WaylandInputButtonEvent {
    uint32_t button;
    uint32_t state;
    uint32_t time;
    struct SDL_WaylandInputButtonEvent* next;
}*/

#define PAN_INTERPOLATION_BUFFER_SIZE 5;

struct SDL_WaylandInputFrame {
  // Implementation notes:
  /*
   * This implementation assumes that every event within
   * a frame doesn't need to be exposed individually,
   * and that rather a frame is a sum, rather than a composite,
   * of the events within it.
   *
   * Benefits:
   *   * we can avoid trying to dynamically allocate for events
   *   * we don't need to buffer for individual events
   *   * any sum attribute (eg scroll deltaX) can have
   *      constant lookup time, no need to iterate and re-sum
   * Drawbacks:
   *   * event order has to be lost within a frame
   *   * any velocity interpolation has to be through
   *      running averages, rather than computed afterwards
   *
   *
   * Ordering of button press events should never actually coincide with
   * any axis events, and has no need to be coupled with axis stop or source
   * events
   * ...and since event ordering is actually undefined at the protocol level for
   * wayland, it doesn't make a difference if we just handle the button press as
   * soon as it comes in, and only couple axis source events
   */
  struct {
    // uint32_t enter : 1; // use later if buffering of enter/leave events
    // happens uint32_t leave : 1;
    uint32_t motion : 1;
    uint32_t axis : 1;
    // uint32_t button : 1; // button can not be summed, so this bitfield is
    // replaced by a linked list
  } events;

  struct {
    int32_t pan_x;
    uint32_t pan_events_x;
    int32_t pan_y;
    uint32_t pan_events_y;

  } pan;

  struct {
    // we keep a running average here so we can do velocity smoothing between
    // actual dispatched events done on each frame.
    struct {
      int x, int y
    } pan_running_buffer[PAN_INTERPOLATION_BUFFER_SIZE];
  } persist;
};

// TODO: revisit, see if any frame elements should be non-zero initialized
static SDL_WaylandInputFrame
    wl_pointer_current_frame; // statically zero initialized
...
static struct pan_sum pointer_axis_get_interpolated_pan()
{
}

static void pointer_handle_axis_common(struct SDL_WaylandInput *input,
...
static void pointer_handle_frame(void *data, struct wl_pointer *wl_pointer) {
  // current decided precedence of events:
  // 1. motion
  // 2. axis*
  //
  // this only actually matters if multiple types of event happen in the same
  // frame, we don't dispatch button press events here since they can't be
  // easily summed and it is unlikely to make a practical difference within a
  // single frame when events get sent. Something to consider for the future,
  // however, is if a pointer_leave event happens and it dispatched before an
  // axis event in particular, behavior may get confusing.
  //
  // consider buffering any pointer_leave event(s) and dispatching them here
  //
  // a weird bug that may show up from that, however, would be if multiple
  // enter/leave events happen within a *single frame*

  if (wl_pointer_current_frame.events.axis) {
    uint32_t direction = SDL_MOUSEWHEEL_NORMAL; // figure out how to detect this from user settings
                                                // it may actually get sent inverted by default
    float pan_x = 0 - (float)wl_fixed_to_double(wl_pointer_current_frame.pan.pan_x);
    float pan_y = 0 - (float)wl_fixed_to_double(wl_pointer_current_frame.pan.pan_y);

    SDL_SendMouseWheel(window->sdlwindow, 0, pan_x, pan_y, direction, flags);
  }

  // reset for the next frame
  wl_pointer_current_frame = {.events = {0}, .pan = {0}};
}
...
static void touch_handler_down(void *data, struct wl_touch *touch,
                               unsigned int serial, unsigned int timestamp,
                               struct wl_surface *surface, int id,
                               wl_fixed_t fx, wl_fixed_t fy) {
  SDL_WindowData *window_data =
      (SDL_WindowData *)wl_surface_get_user_data(surface);
...
static void touch_handler_up(void *data, struct wl_touch *touch,
                             unsigned int serial, unsigned int timestamp,
                             int id) {
  float x = 0, y = 0;
...
static void touch_handler_motion(void *data, struct wl_touch *touch,
                                 unsigned int timestamp, int id, wl_fixed_t fx,
                                 wl_fixed_t fy) {
  SDL_WindowData *window_data =
      (SDL_WindowData *)wl_surface_get_user_data(touch_surface(id));
  const double dblx = wl_fixed_to_double(fx);
  const double dbly = wl_fixed_to_double(fy);
  const float x = dblx / window_data->sdlwindow->w;
  const float y = dbly / window_data->sdlwindow->h;
...
static void keyboard_handle_keymap(void *data, struct wl_keyboard *keyboard,
  ...
  input->xkb.keymap = WAYLAND_xkb_keymap_new_from_string(
      input->display->xkb_context, map_str, XKB_KEYMAP_FORMAT_TEXT_V1, 0);
  munmap(map_str, size);
  close(fd);
...
static void keyboard_handle_key(void *data, struct wl_keyboard *keyboard,
  ...
  if (key < SDL_arraysize(xfree86_scancode_table2)) {
    ...
    if (scancode != SDL_SCANCODE_UNKNOWN)
      SDL_SendKeyboardKey(state == WL_KEYBOARD_KEY_STATE_PRESSED ? SDL_PRESSED
                                                                 : SDL_RELEASED,
                          scancode);
  }
...
static const struct wl_keyboard_listener keyboard_listener = {
    ...
    keyboard_handle_key,    keyboard_handle_modifiers, NULL, /* repeat_info */
};
...
static const struct wl_seat_listener seat_listener = {
    seat_handle_capabilities, NULL, /* name */
};
...
static void data_offer_handle_actions(void *data,
                                      struct wl_data_offer *wl_data_offer,
                                      uint32_t dnd_action) {}
...
static void data_device_handle_enter(void *data,
  ...
  if (id != NULL) {
    ...
    has_mime = Wayland_data_offer_has_mime(data_device->drag_offer, FILE_MIME);

    /* If drag_mime is NULL this will decline the offer */
    wl_data_offer_accept(id, serial, (has_mime == SDL_TRUE) ? FILE_MIME : NULL);

    /* SDL only supports "copy" style drag and drop */
    if (has_mime == SDL_TRUE) {
      dnd_action = WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY;
    }
    wl_data_offer_set_actions(data_device->drag_offer->offer, dnd_action,
                              dnd_action);
  }
}
...
static void data_device_handle_drop(void *data,
  ...
  if (data_device->drag_offer != NULL) {
    /* TODO: SDL Support more mime types */
    buffer = Wayland_data_offer_receive(data_device->drag_offer, &length,
                                        FILE_MIME, SDL_FALSE);
...
static const struct wl_data_device_listener data_device_listener = {
    data_device_handle_data_offer, data_device_handle_enter,
    data_device_handle_leave,      data_device_handle_motion,
    data_device_handle_drop,       data_device_handle_selection};
...
void Wayland_display_add_relative_pointer_manager(SDL_VideoData *d,
                                                  uint32_t id) {
  d->relative_pointer_manager = wl_registry_bind(
      d->registry, id, &zwp_relative_pointer_manager_v1_interface, 1);
}
...
void Wayland_display_add_pointer_constraints(SDL_VideoData *d, uint32_t id) {
  d->pointer_constraints = wl_registry_bind(
      d->registry, id, &zwp_pointer_constraints_v1_interface, 1);
}
...
static void lock_pointer_to_window(SDL_Window *window,
  ...
  locked_pointer = zwp_pointer_constraints_v1_lock_pointer(
      d->pointer_constraints, w->surface, input->pointer, NULL,
      ZWP_POINTER_CONSTRAINTS_V1_LIFETIME_PERSISTENT);
  zwp_locked_pointer_v1_add_listener(locked_pointer, &locked_pointer_listener,
                                     window);
